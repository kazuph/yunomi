...
主担当が渡した正確な差分（基準/対象commit、または未コミット差分）と対象ファイルをレビューする。HEAD~1を勝手に基準にしない。読み取り専用で結果を最終返答へ返し、主担当が明示された報告書へ記録する。秘密値・認証情報・.env本文は出力しない。以下の検索は対象範囲へ絞り、秘密情報の検出結果は値を伏せてファイルと行・種類だけを報告する。
...
- 認証・認可の問題検出
- 機密データ露出の検出
- 結果を最終返答へ返し、主担当がREPORT.mdの「Code & Security Review」へ記録
...
- 重大度が適切に分類されている
- 改善提案が実行可能な形で記載されている
- 主担当へCode & Security Reviewの具体的な結果を返し、報告書への反映担当が明確になっている
