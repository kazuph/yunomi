...
すべての実装・修正は `/do` から `/done` まで同じ完了基準で進める。`/bucho` は同じ成果を委譲する入口。tiny系は統合済みで、規模によるレビュー省略や品質水準の選択はしない。

- `/done` は新要件と既存機能の回帰、Code & Security・E2E・該当UI/UXレビュー、説明図・スクリーンショット・動画、報告検証、人間の承認を保持する。
- 原本は `plugin/skills/{do,done,bucho}/`。配布先は同じ本文を使う。
