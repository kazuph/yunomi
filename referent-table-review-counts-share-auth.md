...
| package version 2.4.1とMCP initialize | npm公開物とMCPクライアントへ返す自己申告を一致させる | initialize結果のserverInfo.version | 値 | MCP initialize要求受信後、応答生成時 | MCP server versionとは実行中npmパッケージと同じ公開版を指す | 2.4.1 |
